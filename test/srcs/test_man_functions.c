...
/*   Updated: 2024/07/02 22:32:06 by alejhern         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	long expected = atoi(nb);
	long result = ft_atoi(nb);
...
{
	unsigned char *expected = (unsigned char *)memchr(s, c, n);
...
void test_strlcat(char *dest, char *src, size_t n)
{
	...
	strlcat(expected, src, n);
	ft_strlcat(result, src, n);

    printf("%s", get_color_test(expected, result, sizeof(char)));
	printf("Function STRLCAT(%s, %s, %zu)\n", dest, src, n);
	printf("\t-> EXPECTED: %s CADENA: %s\n", expected, expected);
	printf("\t-> RESULT:   %s CADENA: %s\n", result, result);
    printf("\033[0m"); // Reset text color
    printf("\n"); 
}
...
{
	char *expected = strnstr(str, to_find, len);
